...
export const getEvent = () => ({
  eventType: [
    ...
    {type: `sightseeing`,
     text: `in`},
    {type: `restaurant`,
     text: `in`}][Math.floor(Math.random() * 10)],
  destination: [
...
export const getTripInfo = eventsList => ({
  get title() {
    ...
    if (tripRoute.length > 3) {
      return new Array(tripRoute.shift(), tripRoute.pop()).join(` — ... — `);
    } else return tripRoute.join(` — `);
  },
  get dates() {
    let tripDates = [...Array(2)];
    tripDates[0] = new Date(eventsList[0].dateStart).toString().slice(4, 10);
    tripDates[1] = new Date(eventsList[eventsList.length - 1].dateEnd).toString().slice(4, 10);
